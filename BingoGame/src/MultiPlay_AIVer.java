...
public class MultiPlay_AIVer extends MultiPlay_NotAIVer{
	...
	void refreshPriorityGrid( BingoGrid bg )
	{
		 ...
		 */
		bg.resetPriorityGrid();	// 해당 객체의 우선순위 배열 초기화. 자리에 따른 우선순위 기본값만 가진 상태. 
		...
		for( int i=0 ; i < BINGOSIZE ; i++ )
		{
			for( int j=0 ; j < BINGOSIZE ; j++ )
			{
				...
				if( bg.checkGrid[i][j] == 1 )
				{
					...
					// 선택된 숫자의 가로, 세로, 대각선(조건이 만족할 경우) 위치에 존재하는 숫자들에게 +10씩 우선순위 높여주고, 빙고된 숫자 갯수 카운트  
					...
					// 체크 갯수가 1개일 경우에는 위에서 이미 체크해줌. 2개 이상일 때부터 아래 문장 수행. 
					if( widthCheckCnt > 1 || heightCheckCnt > 1 || diaLUpCheckCnt > 1 || diaRUpCheckCnt > 1 )
						plusPrioForExtraCheck( bg, widthCheckCnt, heightCheckCnt, diaLUpCheckCnt, diaRUpCheckCnt, i, j);					 

					diaLUpCheckPosition = false;	diaRUpCheckPosition = false;
	...
	// 같은 라인에 존재하는 체크 갯수(ex : 해당 가로줄의 체크갯수 - widthCheckCnt) 만큼 추가점수를 줌. 
	void plusPrioForExtraCheck( BingoGrid bg, int widthCheckCnt, int heightCheckCnt, int  diaLUpCheckCnt, int diaRUpCheckCnt, int i, int j )
	{
		int[] plusForBgCnt = { 0, 5, 10, 25, 45 };	
		// 같은 줄에 빙고가 몇개 있느냐에 따라 추가 점수를 줌. 실질적으로 2번째 값부터 사용. 
	...
	// 제일 높은 우선순위를 가진 숫자 반환. 같은 우선순위를 가진 숫자가 여러개 있을 경우 상대가 가지지 않은 번호 -> 랜덤 순으로 반환. 
	int getHighPriorityNum( BingoGrid myBg, BingoGrid rivalBg )
	{
		...
		saveHighPrioNumToList( myBg, maxNumList );
	...
	// 제일 큰 우선순위를 가진 숫자 찾고 리스트에 저장  
	void saveHighPrioNumToList( BingoGrid bg, ArrayList<Integer> maxNumList )
	{		
		int maxNum = 0 ;
