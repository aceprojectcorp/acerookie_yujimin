...
public class Player : MonoBehaviour {
	...
	void Awake()
	{
		GameData.screenHeight = playUiRoot.manualHeight;
		GameData.screenWidth = playUiRoot.manualWidth;
	...
	void Update () 
	{
		   ...
		   Input.mousePosition.y  > 55f )
		{
			...
			{	
				...
				if( changePlayerPos.x  <=   GameData.screenWidth/2f - 20 &&
				   changePlayerPos.x >= -(GameData.screenWidth/2f - 20 ) )
				{	
	...
	void PlayerInit()
	{
		...
		playerPos.y -= ( GameData.screenHeight / 4f); 			// 플레이어 캐릭터 위치가 0,0 에서 시작하기 때문에 화면 가로크기 1/4만큼 빼줌 
