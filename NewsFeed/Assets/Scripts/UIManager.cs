...
//	public List <GameObject> allFeedList = new List<GameObject>();			// 모든 피드들 저장
	...
	public void UpdateFeedList()
	{
		...
		{ 
			GameObject FeedTodayMissionTmp = Instantiate(Resources.Load("FeedOfMissionToday")) as GameObject;
			initPrefeb( FeedTodayMissionTmp );
			FeedTodayMissionTmp.GetComponent<UISprite>().width  = 800 ;
			FeedTodayMissionTmp.GetComponent<UISprite>().height = 270 ;
			nowEnableFeedList.Add ( FeedTodayMissionTmp );
		...
		if( GameData.Instance.straightWinCnt >= 1 )
		{
...
//		SettingFeed ( nowEnableFeedList [idxFeedList], idxFeedList );
	...
	void initPrefeb( GameObject ins )
	{
		ins.transform.parent = ScrollView.transform ; 
